 ...
 * </PRE>
 * 
 * It may also contain comments or section headers (i.e. anything that does not
 * match the above pattern). Parameter names are not case sensitive. Initial and
 * terminal spaces are trimmed for both parameter names and values. Boolean
 * parameters accept multiple ways of expressing "true" (namely "on", "true",
 * "yes" and "active").
 * <P>
 * 
 * To avoid passing around object handles, this class does not function as an
 * object! There is only one "static object". Example:
 * 
 * <PRE>
...
public class Parameters {
	 ...
	 * value if undefined
	 */
	public static File getFile(String s, File defaultValue)
  ...
  public static File getOrRequestAndAddFile(String s, String description)
	 ...
	 * adds it
	 */
	public static boolean getOrRequestAndAddBoolean(String s, String description)
			throws IOException {
	...
	public static void init(String filename, File... folders)
			throws IOException {
		boolean found = false;
		for (File folder : folders) {
			if (new File(folder, filename).exists()) {
				if (found)
					throw new IOException("INI-file " + filename
							+ "occurs twice in given folders");
	...
	/**
	 * Reports an error message and aborts if the parameters are undefined. p
	 * may contain strings of the form "parametername explanation"
	 */
	...
	public static Database getDatabase() throws Exception {
		Parameters
				.ensureParameters(
						"databaseSystem - either Oracle, Postgres or MySQL",
		...
		if (system.equals("POSTGRES")) {
			return (new PostgresDatabase(user, password, database, host, port,
					schema));
		}
